...
- **Always pass both `--work-tree` and `--git-dir`** — omitting `--work-tree` causes git to fail recognizing bare repo operations
...
- When creating or modifying files under `~/.claude/rules/` or `~/.claude/hooks/`, track them with `git --work-tree=$HOME --git-dir=$HOME/.dotfiles.git add <path>`
