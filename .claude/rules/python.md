...
- Types: all signatures typed; `from __future__ import annotations` at top; `X | None` over `Optional[X]`; `mypy --strict`
...
- Quality: `uv run ruff format --check . && uv run ruff check . && uv run mypy --strict . && uv run pytest`
